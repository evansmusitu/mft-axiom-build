...
mkdir -p "$OUT"
STORE_APK="$OUT/MUSITU_Store_1.0.1.apk"
...
dump_ui_once() {
  ...
  timeout 8s adb pull /sdcard/window.xml "$out" >/dev/null 2>&1 || return 1
  [[ -s "$out" ]]
}
wait_for_ui() {
  ...
  for i in $(seq 1 18); do
    if dump_ui_once "$out" && grep -Fq "$needle" "$out"; then return 0; fi
    sleep 2
  done
  echo "UI text not observed for $package: $needle" >&2
  [[ -f "$out" ]] && cat "$out" >&2 || true
  adb shell dumpsys window windows 2>/dev/null | tail -n 200 >&2 || true
  return 1
}
assert_resumed() {
  local package="$1"
  ...
  for i in $(seq 1 20); do
    dump="$(timeout 8s adb shell dumpsys activity activities 2>/dev/null || true)"
    if printf '%s' "$dump" | grep -E 'mResumedActivity|topResumedActivity|ResumedActivity' | grep -Fq "$package"; then return 0; fi
    sleep 1
  done
  timeout 8s adb shell dumpsys activity activities >&2 || true
  return 1
}
assert_no_package_crash() {
  local package="$1" log="$2"
  timeout 15s adb logcat -d -v threadtime > "$log" || true
...
curl --fail --location --retry 4 --connect-timeout 20 --max-time 120 --silent --show-error "$STORE_URL" -o "$STORE_APK"
test "$(stat -c%s "$STORE_APK")" = "$STORE_BYTES"
test "$(sha_file "$STORE_APK")" = "$STORE_SHA256"

curl --fail --location --retry 4 --connect-timeout 20 --max-time 180 --silent --show-error "$CHEM_URL" -o "$CHEM_APK"
...
grep -q 'versionCode=10001' "$OUT/store-package.txt"
grep -q 'versionName=1.0.1' "$OUT/store-package.txt"
...
e={
  'schema':'musitu.store.phase1.fresh_android_virtual_device.v2',
  ...
  'store':{
    'package_id':'com.musitu.store','version':'1.0.1','version_code':10001,
  ...
  'gate':'MUSITU_STORE_PHASE1_FRESH_ANDROID_VIRTUAL_DEVICE_V101_PASS',
}
