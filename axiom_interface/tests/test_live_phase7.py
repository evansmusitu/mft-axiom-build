...
class Phase7LiveContractTests(unittest.TestCase):
 def test_live_is_wired_into_phase7_shell_and_offline_cache(self):
  ...
  for asset in ["'./live.js'","'./live_durability.js'","'./live_semantic_bridge.js'","'./styles/live.css'"]:self.assertIn(asset,SW)
 ...
 def test_required_modalities_and_browser_media_controls_exist(self):
  ...
  for token in ['getUserMedia','getDisplayMedia','MediaRecorder','data-permission','data-record']:self.assertIn(token,LIVE)
 ...
 def test_accessibility_has_non_drag_controls_live_regions_and_responsive_layout(self):
  ...
  self.assertIn('@media(max-width:52rem)',CSS);self.assertIn('@media(forced-colors:active)',CSS);self.assertIn('.live-privacy .recording',CSS)
