...
import './engineering_command_center_ui.js';
...
async function createAgent(event){
  ...
  try{
    ...
    const db=await open();
    await db.createAgent({projectId,actorId:'local-user',name:$('#vnext-agent-name').value,purpose:$('#vnext-agent-purpose').value,toolScopes:['project.read'],dataScopes:['project.metadata'],autonomy:$('#vnext-agent-autonomy').value,maxRuns:Number($('#vnext-agent-runs').value||10),maxComputeUnits:100});
    $('#vnext-agent-dialog').close();
    await renderAgents();
  }catch(reason){error.textContent=clean(reason?.message||'Agent could not be created',240);error.hidden=false;}
}
...
async function renderAgents(){
  ...
  const projectId=activeProjectId();
  if(!projectId){root.innerHTML=`<header class="page-head"><div><small class="eyebrow">MUSITU AXIOM</small><h1>Agents</h1><p>Governed execution identities with explicit purpose, grants, budgets and kill switches.</p></div></header><div class="card"><h3>Select a project first</h3><p>Agents are always scoped to a Project graph.</p><p style="margin-top:12px"><a class="button primary" href="#/projects">Open projects</a></p></div>`;return;}
  try{
    const db=await open();
  ...
  }catch(reason){root.innerHTML=`<header class="page-head"><div><small class="eyebrow">MUSITU AXIOM</small><h1>Agents</h1><p>Governed execution identities with explicit purpose, grants, budgets and kill switches.</p></div></header><div class="card"><h3>Agent registry unavailable</h3><p>${esc(clean(reason?.message||'Unable to open the browser-local registry',240))}</p></div>`;}
}
...
async function renderAutomations(){
  ...
  const root=$('#workspace-root');if(!root)return;
  const projectId=activeProjectId();
  if(!projectId){root.innerHTML=`<header class="page-head"><div><small class="eyebrow">MUSITU AXIOM</small><h1>Automations</h1><p>Triggered, approved local-preview work bound to governed agents.</p></div></header><div class="card"><h3>Select a project first</h3><p>Automations inherit project and agent authority.</p></div>`;return;}
  try{
    const db=await open();const rows=await db.listAutomations(projectId);
  ...
  }catch(reason){root.innerHTML=`<header class="page-head"><div><small class="eyebrow">MUSITU AXIOM</small><h1>Automations</h1></div></header><div class="card"><h3>Automation registry unavailable</h3><p>${esc(clean(reason?.message||'Unable to open the browser-local registry',240))}</p></div>`;}
}

async function render(){if(route()==='agents')await renderAgents();else if(route()==='automations')await renderAutomations();}
