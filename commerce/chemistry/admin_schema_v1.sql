...
-- Each audit hash may have at most one successor. The application always reads
-- the current head before constructing an event, so this D1-compatible unique
-- index rejects concurrent forks that race with the same previous hash.
CREATE UNIQUE INDEX IF NOT EXISTS idx_chem_admin_audit_prev_hash ON chemistry_admin_audit(prev_hash);
