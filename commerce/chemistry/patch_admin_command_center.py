...
ROUTE_INSERT = "if(p==='/chemistry/admin'||p.startsWith('/chemistry/admin/'))return adminRouter(req,env);"
...
def main() -> None:
    ...
    ap.add_argument('--module',default='commerce/chemistry/admin_command_center_module.mjs')
    ...
    if ROUTE_INSERT in text or 'async function adminRouter(req,env)' in text: raise SystemExit('admin command center already present')
    ...
    for token in REQUIRED_ADMIN:
        if token not in module: raise SystemExit('required admin module behavior missing: '+token)
    candidate=text.replace(MODULE_ANCHOR,module+MODULE_ANCHOR,1)
    ...
    if candidate.count('async function adminRouter(req,env)')!=1: raise SystemExit('admin module insertion mismatch')
