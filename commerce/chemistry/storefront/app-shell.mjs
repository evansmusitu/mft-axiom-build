...
(()=>{
  ...
  const modes=new Set(['equation','ink','structure','mechanism','graph','apparatus','particle','accessibility','graph-data']);
  const blank=()=>({schema:SR_SCHEMA,version:1,examMode:'certified',activeMode:'equation',text:'',accessibilityDescription:'',objects:[],edges:[],ink:[],provenance:[],finalized:false});
  let response=blank();
  try{
    const raw=localStorage.getItem(SR_KEY);
    if(raw){const parsed=JSON.parse(raw);if(parsed&&parsed.schema===SR_SCHEMA&&Array.isArray(parsed.objects)&&Array.isArray(parsed.edges)&&Array.isArray(parsed.ink))response={...blank(),...parsed,activeMode:modes.has(parsed.activeMode)?parsed.activeMode:'equation',finalized:false};}
  }catch{}
  ...
  const board=sr.querySelector('[data-sr-board]');
  const links=sr.querySelector('[data-sr-links]');
  const boardEmpty=sr.querySelector('[data-sr-empty]');
  ...
  const touch=()=>{response.finalized=false;persist();renderSummary()};
  const renderSummary=()=>{
    if(status)status.textContent=response.finalized?'Response locked for review':(response.objects.length||response.edges.length||response.ink.length||response.text||response.accessibilityDescription)?'Local draft saved':'Ready — local draft only';
    if(graphOut)graphOut.textContent=JSON.stringify({schema:response.schema,examMode:response.examMode,text:response.text,objects:response.objects,edges:response.edges,ink:response.ink.map(s=>({tool:s.tool,points:s.points})),accessibilityDescription:response.accessibilityDescription,provenance:response.provenance,finalized:response.finalized},null,2);
  };
  const setMode=mode=>{
    if(!modes.has(mode))return;
    response.activeMode=mode;selected=[];
  ...
  textArea?.addEventListener('input',()=>{response.text=String(textArea.value).slice(0,12000);trace('equation-edit');touch()});
  accessArea?.addEventListener('input',()=>{response.accessibilityDescription=String(accessArea.value).slice(0,6000);trace('accessibility-edit');touch()});
  ...
  const selectNode=id=>{selected=selected.includes(id)?selected.filter(x=>x!==id):[...selected,id].slice(-2);renderBoard();say(selected.length===2?'Two objects selected. Choose a connection.':'Object selected')};
  const addEdge=kind=>{
    if(selected.length!==2){say('Select two objects first');return}
    if(response.edges.length>=MAX_EDGES){say('Connection limit reached');return}
    const [from,to]=selected;if(from===to)return;
    response.edges.push({id:'e'+response.edges.length+'-'+Math.random().toString(36).slice(2,6),mode:response.activeMode,kind:String(kind).slice(0,32),from,to});selected=[];
  ...
  const renderBoard=()=>{
    ...
    board.querySelectorAll('[data-sr-node]').forEach(n=>n.remove());links.replaceChildren();
    const objects=modeObjects(),map=new Map(objects.map(o=>[o.id,o]));
    for(const edge of modeEdges()){
      const a=map.get(edge.from),b=map.get(edge.to);if(!a||!b)continue;
      const x1=a.x,x2=b.x,y1=a.y,y2=b.y;
      if(edge.kind.includes('electron')){
        const path=svgEl('path',{d:'M '+x1+' '+y1+' Q '+((x1+x2)/2)+' '+(Math.min(y1,y2)-14)+' '+x2+' '+y2,'data-edge-kind':edge.kind});links.append(path);
    ...
    for(const o of objects){
      ...
      n.addEventListener('pointerdown',e=>{if(response.finalized)return;drag={id:o.id,pointer:e.pointerId};n.setPointerCapture?.(e.pointerId)});
      n.addEventListener('pointermove',e=>{if(!drag||drag.id!==o.id||drag.pointer!==e.pointerId)return;const r=board.getBoundingClientRect();o.x=Math.max(4,Math.min(92,((e.clientX-r.left)/r.width)*100));o.y=Math.max(7,Math.min(88,((e.clientY-r.top)/r.height)*100));n.style.left=o.x+'%';n.style.top=o.y+'%';renderBoard()});
      n.addEventListener('pointerup',e=>{if(drag?.id===o.id){drag=null;trace('move-object',o.kind);touch()}});board.append(n);
    }
    if(boardEmpty)boardEmpty.hidden=objects.length>0;
  };
  ...
  sr.querySelector('[data-sr-ink-undo]')?.addEventListener('click',()=>{response.ink.pop();trace('ink-undo');touch();drawInk()});
  sr.querySelector('[data-sr-ink-clear]')?.addEventListener('click',()=>{response.ink=[];trace('ink-clear');touch();drawInk()});
  addEventListener('resize',()=>{if(response.activeMode==='ink')resizeInk()},{passive:true});

  sr.querySelector('[data-sr-reset]')?.addEventListener('click',()=>{response=blank();selected=[];textArea&&(textArea.value='');accessArea&&(accessArea.value='');try{localStorage.removeItem(SR_KEY)}catch{}trace('reset');renderBoard();drawInk();setMode('equation');say('Local scientific response cleared')});
  sr.querySelector('[data-sr-finalize]')?.addEventListener('click',()=>{response.finalized=true;trace('finalize');persist();renderSummary();say('Response locked for review. No network submission has occurred.')});
  sr.querySelector('[data-sr-reopen]')?.addEventListener('click',()=>{response.finalized=false;trace('reopen');persist();renderSummary();say('Response reopened for editing')});
  ...
  textArea&&(textArea.value=response.text||'');accessArea&&(accessArea.value=response.accessibilityDescription||'');
  setMode(response.activeMode||'equation');renderBoard();renderSummary();
})();
`;
