...
SCHEMA = "musitu.axiom.openai-provider-metadata-probe.v1"
DEFAULT_MODEL = "gpt-5.6-sol"
PROBE_TEXT = "Return exactly MUSITU_OPENAI_PROVIDER_PROBE_OK and nothing else."
EXPECTED_TEXT = "MUSITU_OPENAI_PROVIDER_PROBE_OK"
EVALUATION_CANDIDATE_SHA = "eb5f026b165d558df7112c2880014d6019019be7"
SAFE_HEADERS = frozenset({
    "content-type", "date", "x-request-id", "openai-organization",
    "openai-project", "openai-version", "openai-processing-ms",
})
...
def _canonical_api_key(value: str) -> str:
    ...
    try:
        ...
        raise ValueError("OPENAI_API_KEY contains unsupported non-ASCII characters") from exc
    if any(ord(ch) < 33 or ord(ch) > 126 for ch in normalized):
        raise ValueError("OPENAI_API_KEY contains unsupported characters")
    return normalized
...
def _safe_error_category(raw: bytes) -> tuple[str | None, str]:
    try:
    ...
    type_fold = (error_type or "").casefold()
    if any(token in code_fold or token in type_fold for token in ("insufficient_quota", "billing", "credit", "hard_limit")):
        return error_type, "credit_or_billing_limit"
    if any(token in code_fold or token in type_fold for token in ("invalid_api_key", "authentication", "unauthorized")):
        return error_type, "authentication_error"
    if any(token in code_fold for token in ("model_not_found", "model_not_available", "unsupported_model")):
        return error_type, "model_access_or_availability"
    if any(token in code_fold or token in type_fold for token in ("rate_limit", "rate_limit_exceeded")):
        return error_type, "rate_limit"
    return error_type, "provider_rejected_request"
...
def run_probe(*, api_key: str, model: str = DEFAULT_MODEL, timeout_seconds: int = 45, opener=urlopen) -> dict[str, Any]:
    ...
    body_obj = {"model": model, "input": PROBE_TEXT, "max_output_tokens": 48}
    ...
    try:
        ...
        error_type, error_category = _safe_error_category(raw)
        return {
            "schema": SCHEMA,
            "status": "HTTP_ERROR",
            "provider_org": "OpenAI",
            ...
            "access_mode": "zero_budget_probe",
            "evaluation_candidate_sha": EVALUATION_CANDIDATE_SHA,
            "started_at": started_at,
            "completed_at": _now(),
            "http_status": int(exc.code),
            "request_sha256": request_sha256,
            "error_body_sha256": _sha256_bytes(raw),
    ...
    identity_ready = bool(request_id and response_id and model_version)
    call_valid = bool(200 <= status < 300 and response_id and model_version and text == EXPECTED_TEXT)
    return {
        "schema": SCHEMA,
        "status": "PASS" if call_valid else "FAIL",
        "provider_org": "OpenAI",
        "product": "OpenAI API",
        "requested_model": model,
        ...
        "request_sha256": request_sha256,
        "raw_response_sha256": raw_hash,
        "provider_receipt_hash": _sha256_bytes(_canonical_bytes(receipt)),
        "provider_request_id": request_id,
        "provider_response_id": response_id,
        "safe_response_headers": headers,
        "response_matches_probe": text == EXPECTED_TEXT,
        "level5_identity_ready": identity_ready,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not raw_key:
        payload = {
            ...
            "claim_authority": "NONE",
            "reasons": ["OPENAI_API_KEY_missing"],
        }
    ...
    try:
        ...
        payload = {
            ...
            "evaluation_candidate_sha": EVALUATION_CANDIDATE_SHA,
            "level5_identity_ready": False,
            "level5_admissibility": "NOT_ADMISSIBLE_INVALID_API_KEY_ENCODING",
            "claim_authority": "NONE",
            "reasons": [str(exc)],
        }
    _write(args.output, payload)
    print(json.dumps({
        ...
        "provider_error_type": payload.get("provider_error_type"),
