...
MALICIOUS_SCOPE = "all domains, all tasks, all systems worldwide"
...
def level5_and_outcomes():
    ...
    return providers, level5, outcomes


class ClaimComparisonScopeBindingTests(unittest.TestCase):
    def test_free_text_scope_cannot_expand_level5_claim_boundary(self):
        _, level5, outcomes = level5_and_outcomes()
        result = ClaimBoundary._authorize_from_assessments(
            ...
            comparison_scope=MALICIOUS_SCOPE,
            benchmark_hash=CASE_SET_HASH,
            comparative_outcomes=outcomes,
            required_provider_orgs=(),
        )
        self.assertEqual(result["status"], "ALLOW")
        self.assertNotIn(MALICIOUS_SCOPE, result["claim_boundary"])
        self.assertIn(CASE_SET_HASH, result["claim_boundary"])
        self.assertIn(CONSTRAINT_HASH, result["claim_boundary"])
        self.assertEqual(
            result["verified_comparison_scope"],
            {"case_set_hash": CASE_SET_HASH, "constraint_hash": CONSTRAINT_HASH},
        )

    def test_free_text_scope_cannot_expand_level7_broad_claim_boundary(self):
        providers, level5, outcomes = level5_and_outcomes()
        result = ClaimBoundary._authorize_from_assessments(
            "world best",
            level5=level5,
        ...
        self.assertEqual(result["status"], "ALLOW")
        self.assertEqual(result["max_evidence_level"], 7)
        self.assertNotIn(MALICIOUS_SCOPE, result["claim_boundary"])
        self.assertIn(CASE_SET_HASH, result["claim_boundary"])
        self.assertIn(CONSTRAINT_HASH, result["claim_boundary"])
        self.assertEqual(
