...
async function hexBytes(bytes){const d=await crypto.subtle.digest("SHA-256",bytes);return [...new Uint8Array(d)].map(x=>x.toString(16).padStart(2,"0")).join("")}
async function authorityCall(bytes,env){return env.PAYMENT_AUTHORITY.fetch("https://payment-authority.internal/internal/paynow/verify-callback",{method:"POST",headers:{authorization:"Bearer "+env.BILLING_BRIDGE_CAPABILITY_TOKEN,"content-type":"application/json"},body:JSON.stringify({raw_body_base64:b64(bytes)})})}
async function probe(env){try{const raw=new TextEncoder().encode("status=Paid&reference=musitu-probe&hash="+"0".repeat(128));const r=await authorityCall(raw,env);const t=await r.text();let o={};try{o=JSON.parse(t)}catch{}return{reachable:true,bridge_auth_accepted:r.status!==401&&r.status!==403,verifier_responded:r.ok&&typeof o.valid==="boolean",status:r.status}}catch{return{reachable:false,bridge_auth_accepted:false,verifier_responded:false,status:null}}}
async function verify(bytes,env){let r;try{r=await authorityCall(bytes,env)}catch{return{ok:false,kind:"unreachable"}}const text=await r.text();let o={};try{o=JSON.parse(text)}catch{}if(r.status===401||r.status===403)return{ok:false,kind:"auth"};if(!r.ok)return{ok:false,kind:"upstream"};if(o?.valid!==true)return{ok:false,kind:"invalid"};return{ok:true,callback_sha256:typeof o.callback_sha256==="string"?o.callback_sha256:null}}
async function health(env){let d1=false;try{const r=await env.AXIOM_DB.prepare("SELECT 1 AS ok").first();d1=Number(r?.ok)===1}catch{}const a=await probe(env);const ok=d1&&a.reachable&&a.bridge_auth_accepted&&a.verifier_responded;return out(ok?200:503,{ok,service:"MUSITU Axiom Billing Ingress",mode:"catalog_fail_closed",catalog_configured:false,charges_enabled:false,subscription_activation_enabled:false,d1,authority:a,transport_bound:Boolean(env.PAYNOW_TRANSPORT)})}
async function webhook(req,env){const ct=(req.headers.get("content-type")||"").toLowerCase();if(!ct.startsWith("application/x-www-form-urlencoded"))return out(415,{ok:false,error:"unsupported_media_type"});if(Number(req.headers.get("content-length")||0)>65536)return out(413,{ok:false,error:"payload_too_large"});const bytes=new Uint8Array(await req.arrayBuffer());if(bytes.length>65536)return out(413,{ok:false,error:"payload_too_large"});const v=await verify(bytes,env);if(!v.ok)return out(v.kind==="invalid"?400:502,{ok:false,error:v.kind==="invalid"?"invalid_paynow_callback":"payment_authority_unavailable"});const digest=await hexBytes(bytes),now=new Date().toISOString();if(v.callback_sha256&&v.callback_sha256!==digest)return out(502,{ok:false,error:"authority_digest_mismatch"});const payload=JSON.stringify({verified:true,body_sha256:digest,authority_digest_matched:true,raw_payload_stored:false,catalog_mapped:false,subscription_mutated:false});await env.AXIOM_DB.prepare("INSERT OR IGNORE INTO webhook_events (id,provider,event_type,payload_json,processed_at) VALUES (?1,?2,?3,?4,?5)").bind("paynow_"+digest,"paynow","paynow.callback.verified.unmapped",payload,now).run();return out(202,{ok:true,verified:true,processed:false,subscription_mutated:false,reason:"catalog_not_configured"})}
...
try:
    ...
    def pub(path,method='GET',body=None,ctype=None):
        h={'Accept':'application/json','User-Agent':'MUSITU-Axiom-Billing-Deploy-Probe/2.0'}
        if ctype:h['Content-Type']=ctype
        return req(BASE+path,method,h,body,30)
    c,_,b=pub('/billing/healthz'); health=json.loads(b or b'{}')
    if c!=200 or health.get('ok') is not True or health.get('catalog_configured') is not False or health.get('charges_enabled') is not False or health.get('authority',{}).get('bridge_auth_accepted') is not True or health.get('authority',{}).get('verifier_responded') is not True:raise RuntimeError('health contract failed '+str(c)+' '+b[:400].decode('utf-8','ignore'))
    ...
    invalid=('status=Paid&reference=invalid&hash='+'0'*128).encode()
    c,_,_=pub('/billing/webhooks/paynow','POST',invalid,'application/x-www-form-urlencoded')
    if c!=400:raise RuntimeError('invalid webhook expected 400 got '+str(c))
    ...
    c,_,b=req(BASE+'/',headers={'Accept':'application/json','User-Agent':'MUSITU-Axiom-Billing-Deploy-Probe/2.0'}); root=json.loads(b or b'{}')
    ...
    c,_,_=req(BASE+'/internal/paynow/poll','POST',{'Content-Type':'application/json','User-Agent':'MUSITU-Axiom-Billing-Deploy-Probe/2.0'},b'{}')
