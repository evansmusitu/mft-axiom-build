...
def relevance_calibration_metrics(rows, k, th):
    if not rows:
        return {'relevance_detection_f1': 0.0, 'relevance_accuracy': 0.0, 'false_grounding_notmentioned': 0.0, 'positive_relevance_recall': 0.0, 'selected_evidence_exact_span_recall': 0.0, 'n': 0}
    ...
    evhit = sum(rows[i]['scores'][str(k)]['best_span_index'] in rows[i]['gold_spans'] for i in pos) / (len(pos) or 1)
    return {
...
def main():
    ...
    tensor_counts, name_hashes, shape_hashes = set(), set(), set()
    for f in files:
        o = json.load(open(f))
        if o.get('schema') != 'musitu.revenueguard.contractnli.decoupled_raw_shard.v2':
        ...
        tensor_counts.add(int(o.get('trainable_tensor_count', -1)))
        name_hashes.add(o.get('trainable_names_sha256'))
        shape_hashes.add(o.get('trainable_shapes_sha256'))
    ...
    if tensor_counts != {EXPECTED_TRAINABLE_COUNT} or name_hashes != {EXPECTED_TRAINABLE_NAMES_SHA256} or shape_hashes != {EXPECTED_TRAINABLE_SHAPES_SHA256}:
        raise SystemExit(f'trainable state fingerprint mismatch counts={tensor_counts} names={name_hashes} shapes={shape_hashes}')
    ...
    for k in KS:
        ...
        for th in THRESHOLDS:
            ...
            key = (m['false_grounding_notmentioned'] <= .02, m['relevance_detection_f1'], m['positive_relevance_recall'], m['relevance_accuracy'], m['selected_evidence_exact_span_recall'], -m['false_grounding_notmentioned'], -th)
    ...
    def setting_key(x):
        ...
        return (m['false_grounding_notmentioned'] <= .02, m['relevance_detection_f1'], m['positive_relevance_recall'], m['relevance_accuracy'], m['selected_evidence_exact_span_recall'], -m['false_grounding_notmentioned'], -x['k'], -x['threshold'])
    ...
    gate['pass'] = dm['accuracy'] >= .90 and dm['macro_f1'] >= .88 and dm['false_grounding_notmentioned'] <= .02 and dm['selected_evidence_exact_span_recall'] >= .85
    ...
    rep = {
        ...
        'completed_chunks': 4,
        'trainable_tensor_count': EXPECTED_TRAINABLE_COUNT,
        'trainable_names_sha256': EXPECTED_TRAINABLE_NAMES_SHA256,
        'trainable_shapes_sha256': EXPECTED_TRAINABLE_SHAPES_SHA256,
