...
def main():
    ...
    rows=[];seen_shards=set();seen=set();steps=set();next_chunks=set()
    for f in files:
        o=json.load(open(f))
        if o.get('schema')!='musitu.revenueguard.contractnli.checkpoint_raw_shard.v1':raise SystemExit(f'bad schema {f}')
        ...
        seen_shards.add(si);steps.add(o['global_training_steps']);next_chunks.add(o['next_chunk'])
        for r in o['rows']:
    ...
    rep={'schema':'musitu.revenueguard.contractnli.checkpoint_eval.v1','status':'DEV_RESEARCH_ONLY','mode':args.mode,'checkpoint_artifact_sha256':args.checkpoint_artifact_sha256,'global_training_steps':next(iter(steps)),'completed_chunks':next(iter(next_chunks)),'calibration_rows':len(cal),'dev_rows':len(dev),'candidate_settings':settings,'chosen_k':chosen['k'],'chosen_threshold':chosen['threshold'],'selection_rule':'K and threshold selected exclusively on hash-held training calibration documents; untouched dev graded once after freeze','dev':dm,'dev_by_hypothesis':by,'gate':gate}
