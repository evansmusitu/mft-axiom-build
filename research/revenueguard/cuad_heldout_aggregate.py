...
def main():
    ...
    rows=[];seen_shards=set();seen_ids=set();scoring_modes=set()
    for f in files:
        o=json.load(open(f));
        ...
        scoring_modes.add(o.get('scoring_mode','legacy_span_null_confidence'))
        si=o['shard_index']
    ...
    if len(scoring_modes)!=1:raise SystemExit(f'mixed scoring modes {sorted(scoring_modes)}')
