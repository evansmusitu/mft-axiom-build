...
def shard_for(title:str,n:int)->int:
    h=hashlib.sha256(('musitu-cuad-eval-shard-v1|'+title).encode()).hexdigest()
    return int(h[:8],16)%n
...
def main():
    ...
    model=AutoModelForQuestionAnswering.from_pretrained(model_path,local_files_only=True);model.eval()
    raw=json.load(open(args.train_json))['data']
    rows=[];titles=[]
    for contract in raw:
        title=contract.get('title','')
        if not primary_dev(title) or shard_for(title,args.num_shards)!=args.shard_index:continue
        titles.append(title)
        ...
        for para in contract['paragraphs']:
            ...
            for qa in para['qas']:
                ...
                rows.append({
                    'title':title,'split':split,'id':qa['id'],'has_answer':bool(answers),
                    'gold_texts':[a['text'] for a in answers],
                    'gold_spans':[(a['answer_start'],a['answer_start']+len(a['text'])) for a in answers],
                    **scored
                })
    rep={'schema':'musitu.revenueguard.cuad.heldout_raw_shard.v1','status':'DEV_RESEARCH_ONLY','shard_index':args.shard_index,'num_shards':args.num_shards,'title_count':len(set(titles)),'row_count':len(rows),'rows':rows}
