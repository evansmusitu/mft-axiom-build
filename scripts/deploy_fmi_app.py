...
AUTH={'X-Auth-Email':os.environ['CLOUDFLARE_EMAIL'],'X-Auth-Key':os.environ['CLOUDFLARE_GLOBAL_API_KEY'],'Accept':'application/json','User-Agent':'MUSITU-FMI-App-Deploy/1.1'}
...
def json_probe(url,method='GET',obj=None,headers=None):
    h={'accept':'application/json','user-agent':'MUSITU-FMI-App-Acceptance/1.1'}
...
if 'live_trading_authorized:false' not in text.replace(' ',''):raise RuntimeError('authority boundary missing')
...
if isinstance(upload_obj,dict) and upload_obj.get('success') is False:raise RuntimeError('app Worker upload success=false '+str(upload_obj.get('errors'))[:500])

settings=None;settings_error=None
for _ in range(30):
    try:
        settings=cf(f'/accounts/{AID}/workers/scripts/{urllib.parse.quote(WORKER,safe="")}/settings') or {};settings_error=None;break
    except RuntimeError as e:
        settings_error=e
        if 'HTTP 404' not in str(e):raise
        time.sleep(1)
if settings is None:raise RuntimeError('new Worker settings never became readable: '+str(settings_error))
...
if substate.get('enabled') is not False or substate.get('previews_enabled') is not False:raise RuntimeError('app workers.dev/previews not disabled')

base='https://'+HOST;health=None
for _ in range(35):
    hc,hh,ho,_=json_probe(base+'/healthz?ts='+str(time.time_ns()))
    if hc==200 and isinstance(ho,dict) and ho.get('ok') is True and ho.get('authority')=='PAPER_SHADOW_ONLY' and ((ho.get('upstream') or {}).get('status')==200):health=(hc,hh,ho);break
    time.sleep(1)
if health is None:raise RuntimeError('canonical app health acceptance failed')
...
if not required_headers.issubset(set(health[1])):raise RuntimeError('canonical app security headers incomplete')
...
for path,ctype in [('/app','text/html'),('/app.css','text/css'),('/app.js','application/javascript'),('/sw.js','application/javascript'),('/manifest.webmanifest','application/manifest+json'),('/icon.svg','image/svg+xml')]:
    sc,sh,sb=raw(base+path+'?ts='+str(time.time_ns()),headers={'user-agent':'MUSITU-FMI-App-Acceptance/1.1'},timeout=30)
    ...
    if not required_headers.issubset(set(sh)):raise RuntimeError('app asset security headers incomplete '+path)
...
out={'schema':'musitu.fmi.customer-app-v1.deployment.v2','gate':'FMI_CUSTOMER_APP_V1_READY','worker':WORKER,'canonical_url':base+'/app','host':HOST,'source_sha256':SOURCE_SHA,'build':'MUSITU_FMI_APP_V1_20260915','upstream':PUBLIC_BASE,'workers_dev_enabled':False,'previews_enabled':False,'custom_domain_service':after[0].get('service'),'custom_domain_id':after[0].get('id'),'canonical_health_http':health[0],'upstream_health_http':(health[2].get('upstream') or {}).get('status'),'authority':health[2].get('authority'),'live_trading_authorized':False,'static_assets':static,'proxy_health_http':pc,'unauthenticated_me_http':mc,'unauthenticated_analysis_http':ac,'content_v2_http':cc,'state_preservation':{'d1_api_calls_performed':False,'customer_rows_modified':False,'billing_rows_modified':False,'payment_provider_called':False,'checkout_initiated':False,'admin_worker_modified':False,'public_edge_worker_modified':False},'certification_boundaries':{'authority':'PAPER_SHADOW_ONLY','profitability':'NOT_YET_CERTIFIED','superiority':'NOT_CERTIFIED','real_money_settlement':'NOT_CERTIFIED'}}
...
print(json.dumps({'gate':out['gate'],'canonical_url':out['canonical_url'],'canonical_health_http':health[0],'upstream_health_http':out['upstream_health_http'],'proxy_health_http':pc,'unauthenticated_me_http':mc,'unauthenticated_analysis_http':ac,'workers_dev_enabled':False,'source_sha256':SOURCE_SHA,'evidence_sha256':dg,'d1_api_calls_performed':False,'checkout_initiated':False},sort_keys=True))
