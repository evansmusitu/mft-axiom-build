...
CFH={'X-Auth-Email':os.environ['CLOUDFLARE_EMAIL'],'X-Auth-Key':os.environ['CLOUDFLARE_API_KEY'],'Accept':'application/json','User-Agent':'MUSITU-FMI-Pending-Settlement-Reconcile/1.0'}
...
def api(path,obj,bearer):
 h={'Accept':'application/json','Content-Type':'application/json','Authorization':'Bearer '+bearer,'User-Agent':'MUSITU-FMI-Pending-Settlement-Reconcile/1.0'}
...
def cleanup(customer,kid):
 d1('DELETE FROM api_keys WHERE id=?1',[kid]); n=int(one("SELECT count(*) AS n FROM api_keys WHERE customer_id=?1 AND status='active'",[customer])['n'])
...
remain=int(one("SELECT count(*) AS n FROM api_keys k JOIN customers c ON c.id=k.customer_id WHERE k.id LIKE 'settle_%' AND k.status='active'",[])['n'])
