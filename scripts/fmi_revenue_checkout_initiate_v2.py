import datetime, hashlib, json, os, pathlib, urllib.error, urllib.parse, urllib.request, uuid
...
CF_HEADERS = {
    ...
    'User-Agent': 'MUSITU-FMI-Revenue-Checkout-Initiate/2.0',
}
...
def api(path, method='GET', obj=None, bearer=None):
    headers = {'Accept': 'application/json', 'User-Agent': 'MUSITU-FMI-Revenue-Checkout-Initiate/2.0'}
    body = None
    if obj is not None:
...
def d1(sql, params=None):
    headers = dict(CF_HEADERS); headers['Content-Type'] = 'application/json'
    obj = {'sql': sql}
...
def mask(value):
    if value: print('::add-mask::' + str(value))
...
existing = d1(
    "SELECT i.reference,i.status,i.expires_at FROM billing_checkout_intents i "
    "JOIN customers c ON c.id=i.customer_id WHERE c.email LIKE ?1 ORDER BY i.created_at DESC",
    [CERT_EMAIL_PREFIX + '%']
)
for row in existing:
...
email = f'{CERT_EMAIL_PREFIX}{suffix}@invalid.example'
customer_id = None; api_key = None; reference = None; browser_url = None; provider_created = False
...
try:
    ...
    if len(rows) != 1: raise RuntimeError('FMI checkout intent not persisted exactly once')
    r = rows[0]
    if r.get('customer_id') != customer_id: raise RuntimeError('FMI checkout customer mismatch')
    ...
    if not exp or exp <= datetime.datetime.now(datetime.timezone.utc): raise RuntimeError('FMI checkout expiry invalid/stale')

    cust = d1('SELECT email,plan,status FROM customers WHERE id=?1 LIMIT 2', [customer_id])
    if len(cust) != 1 or cust[0].get('email') != email or str(cust[0].get('plan') or '').upper() != 'FREE' or cust[0].get('status') != 'active':
    ...
    if int(subs[0]['n']) != 0: raise RuntimeError('FMI unpaid checkout created active subscription')
    acts = d1("SELECT count(*) AS n FROM lifecycle_events WHERE customer_id=?1 AND event_name='PLAN_ACTIVATED'", [customer_id])
    if int(acts[0]['n']) != 0: raise RuntimeError('FMI unpaid checkout emitted PLAN_ACTIVATED')
    ...
    if int(remaining[0]['n']) != 0: raise RuntimeError('FMI certification identity retained API keys')
    ...
    evidence = {
        'schema':'musitu.fmi.real-money-certification-checkout-initiation.v2',
        'gate':'FMI_REAL_MONEY_CERTIFICATION_CHECKOUT_READY',
        'plan':PLAN,'price_id':EXPECTED_PRICE_ID,'amount_cents':EXPECTED_AMOUNT,'currency':EXPECTED_CURRENCY,
        'provider':'paynow','provider_checkout_created':True,'payment_completed':False,'intent_status':r.get('status'),
        'checkout_http':checkout_code,'checkout_not_expired':True,
        'reference_sha256':digest(reference),'customer_id_sha256':digest(customer_id),
        'temporary_signup_api_keys_deleted':True,'active_subscription_count':0,'plan_activated_event_count':0,
        'customer_plan_before_payment':'FREE','raw_api_key_published':False,'raw_customer_id_published':False,
        'raw_reference_published':False,'payer_url_logged':False,'payer_url_artifact_only':True,
        'production_worker_mutation':False,'live_trading_authorized':False,'trade_execution_authorized':False,
        'profitability':'NOT_YET_CERTIFIED','superiority':'NOT_CERTIFIED'
    }
    ...
    pathlib.Path('fmi-real-money-certification-checkout-initiation.sha256').write_text(ev_sha+'  fmi-real-money-certification-checkout-initiation.json\n', encoding='utf-8')
    print(json.dumps({'gate':evidence['gate'],'plan':PLAN,'amount_cents':EXPECTED_AMOUNT,'currency':EXPECTED_CURRENCY,
                      'provider_checkout_created':True,'payment_completed':False,'checkout_not_expired':True,
                      'temporary_signup_api_keys_deleted':True,'raw_api_key_published':False,
                      'raw_customer_id_published':False,'raw_reference_published':False,'payer_url_logged':False,
                      'evidence_sha256':ev_sha}, sort_keys=True))
finally:
    if customer_id and not provider_created:
        d1('DELETE FROM api_keys WHERE customer_id=?1', [customer_id])
        d1('DELETE FROM customers WHERE id=?1', [customer_id])
        left = d1('SELECT count(*) AS n FROM customers WHERE id=?1', [customer_id])
        if int(left[0]['n']) != 0:
            raise RuntimeError('failed FMI pre-checkout disposable customer cleanup')
        print(json.dumps({'precheckout_cleanup_verified':True}, sort_keys=True))
