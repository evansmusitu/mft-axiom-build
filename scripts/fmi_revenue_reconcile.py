...
CFH={'X-Auth-Email':os.environ['CLOUDFLARE_EMAIL'],'X-Auth-Key':os.environ['CLOUDFLARE_API_KEY'],'Accept':'application/json','User-Agent':'MUSITU-FMI-Real-Money-Reconcile/1.0'}
...
def api(path,method='GET',obj=None,bearer=None):
    h={'Accept':'application/json','User-Agent':'MUSITU-FMI-Real-Money-Reconcile/1.0'};body=None
    if obj is not None:
...
# Only the latest fresh certification checkout may be reconciled. Older rows are allowed but not targeted.
exp=parse_dt(intent.get('expires_at'))
if str(intent.get('intent_status') or '').lower()!='paid' and exp and exp<=utcnow():raise RuntimeError('latest FMI certification checkout expired before reconciliation')
...
try:
    ...
    if not paid:
        ...
        evidence={
            'schema':'musitu.fmi.real-money-certification-reconcile.v1',
        ...
        evidence={
            'schema':'musitu.fmi.real-money-certification-reconcile.v1','gate':'FMI_REAL_MONEY_E2E_PASS',
    ...
    # This certification key must never remain active after the run.
...
# Safe hashes only; never output raw identifiers.
