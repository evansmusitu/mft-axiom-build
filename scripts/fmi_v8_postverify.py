...
H={'X-Auth-Email':os.environ['CLOUDFLARE_EMAIL'],'X-Auth-Key':os.environ['CLOUDFLARE_API_KEY'],'User-Agent':'MUSITU-FMI-V8-IndependentReadback/1.1'}
...
def public(path,method='GET',obj=None,bearer=None):
    h={'Accept':'application/json','User-Agent':'MUSITU-FMI-V8-LiveProbe/1.1'}; body=None
...
def d1(sql,params=None):
    hh=dict(H);hh['Content-Type']='application/json';q={'sql':sql}
    ...
    if code!=200:raise RuntimeError('D1 HTTP '+str(code))
    x=json.loads(body or b'{}')
    if x.get('success') is False or any(r.get('success') is not True for r in (x.get('result') or [])):raise RuntimeError('D1 success=false')
    rows=[]
    for r in x.get('result') or []:rows.extend(r.get('results') or [])
    return rows

def cleanup(cid):
    if not cid:return
    d1('DELETE FROM customers WHERE id=?1',[cid])
    rows=d1('SELECT COUNT(*) AS n FROM customers WHERE id=?1',[cid])
    if not rows or int(rows[0].get('n',-1))!=0:raise RuntimeError('promotion fixture cleanup verification failed')

edge_src=src(os.environ['FMI_EDGE']); billing_src=src(os.environ['FMI_BILLING']); adapter_src=src(os.environ['FMI_ADAPTER'])
edge_sha=hashlib.sha256(edge_src).hexdigest(); billing_sha=hashlib.sha256(billing_src).hexdigest(); adapter_sha=hashlib.sha256(adapter_src).hexdigest()
...
if adapter_sha!=os.environ['EXPECTED_ADAPTER_SHA']: raise SystemExit(f'Fail-closed adapter Worker changed: {adapter_sha}')
b=settings(os.environ['FMI_EDGE'])
if set(b)!={'FMI_DB','FMI_KERNEL','FMI_BILLING'}:raise SystemExit('Fail-closed edge binding-name drift')
...
es=subdomain(os.environ['FMI_EDGE']); bs=subdomain(os.environ['FMI_BILLING']); ads=subdomain(os.environ['FMI_ADAPTER'])
if es.get('enabled') is not True or es.get('previews_enabled') is not False:raise SystemExit('Fail-closed edge exposure policy drift')
if bs.get('enabled') is not False or ads.get('enabled') is not False:raise SystemExit('Fail-closed private Worker exposure drift')
...
email_addr='fmi-v8-promotion-'+uuid.uuid4().hex+'@invalid.example';cid=None;cleanup_ok=False
try:
    sc,signup,_,_=public('/v1/signup','POST',{'email':email_addr})
    if sc!=201 or not isinstance(signup,dict) or not signup.get('api_key') or not signup.get('customer_id') or signup.get('plan')!='FREE': raise RuntimeError('Fail-closed free-signup promotion probe')
    ...
    if mc!=201 or not isinstance(mj,dict) or len(str((mj.get('monitor') or {}).get('rule_sha256','')))!=64: raise RuntimeError('Fail-closed V8 monitor creation probe')
    ...
    if a1!=200 or not isinstance(x1,dict) or x1.get('classification')!='PAPER_SHADOW_MARKET_INTELLIGENCE': raise RuntimeError('Fail-closed first V8 analysis probe: HTTP '+str(a1))
    if h1.get('x-fmi-evidence-indexed')!='true': raise RuntimeError('Fail-closed V8 evidence indexing probe')
    ...
    if a2!=200 or h2.get('x-fmi-monitor-evaluation')!='OK': raise RuntimeError('Fail-closed V8 monitor evaluation probe: HTTP '+str(a2)+' status '+str(h2.get('x-fmi-monitor-evaluation')))
    ec,events,_,_=public('/v1/evidence/monitor-events?limit=50','GET',None,token)
    if ec!=200 or not isinstance(events,dict) or events.get('background_market_polling') is not False or events.get('delivery')!='IN_APP_ON_NEW_EVIDENCE': raise RuntimeError('Fail-closed V8 monitor feed contract')
finally:
    if cid:
        cleanup(cid);cleanup_ok=True
if not cleanup_ok:raise SystemExit('Fail-closed promotion fixture cleanup did not execute')
ev={'schema':'musitu-fmi.v8-live-promotion-evidence.v2','gate':'PASS','deployment_pattern':'AXIOM_CLOUDFLARE_CONTENT_PUT_WITH_EXACT_ROLLBACK','worker':os.environ['FMI_EDGE'],'public_base':BASE,'edge_sha256':edge_sha,'workspace_asset_sha256':workspace_sha,'billing_worker_sha256':billing_sha,'adapter_worker_sha256':adapter_sha,'d1_uuid':os.environ['FMI_DB_UUID'],'bindings':{'FMI_DB':'PASS','FMI_KERNEL':'PASS','FMI_BILLING':'PASS','FMI_MARKET_DATA':'NOT_BOUND_MANUAL_FALLBACK'},'edge_workers_dev':True,'edge_previews_enabled':False,'private_workers_public':False,'health':'PASS','security_headers':'PASS','unauthenticated_analysis_401':'PASS','free_signup_probe':'PASS_AND_CLEANED','v8_monitor_creation':'PASS','v8_evidence_indexing':'PASS','v8_monitor_evaluation':'PASS','monitor_delivery':'IN_APP_ON_NEW_EVIDENCE','background_market_polling':False,'numeric_finance_risk_authority':'DETERMINISTIC_TOOL_ONLY','live_trading_authorized':False,'trade_execution_authorized':False,'production_model_authority':False,'scaled_autonomy':False,'profitability':'NOT_YET_CERTIFIED','superiority':'NOT_CERTIFIED','frontier':'NOT_YET_CERTIFIED','real_money_settlement':'NOT_CERTIFIED','billing_worker_modified':False,'payment_action_executed':False,'secret_values_read_or_logged':False}
...
print(json.dumps({'gate':'PASS','edge_sha256':edge_sha,'workspace_sha256':workspace_sha,'billing_sha256':billing_sha,'adapter_sha256':adapter_sha,'fixture_cleanup':'PASS','evidence_sha256':ev['evidence_sha256']},sort_keys=True))
