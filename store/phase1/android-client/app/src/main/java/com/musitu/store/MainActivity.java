...
public class MainActivity extends Activity {
  final ExecutorService io=Executors.newSingleThreadExecutor(); LinearLayout root; TextView status,meta; Button primary,repair,rollback,transfer,refresh; JSONObject catalog,release;
  @Override public void onCreate(Bundle b){super.onCreate(b);renderShell();scheduleUpdates();refresh();}
  void renderShell(){
  ...
  void apply(JSONObject c,JSONObject r){catalog=c;release=r;refresh.setEnabled(true);long installed=StoreCore.installedVersion(this);long current=r.optLong("versionCode",-1);JSONObject a=r.optJSONObject("android");status.setText(installed<0?"Not installed":installed<current?"Update available":"Installed · up to date");
    ...
    primary.setEnabled(true);repair.setEnabled(true);rollback.setEnabled(r.optJSONObject("rollback")!=null&&r.optJSONObject("rollback").optBoolean("authorized",false));primary.setText(installed<0?"Install":installed<current?"Update":"Open");
  }
  ...
  void downloadAndInstall(){if(release==null)return;JSONObject a=release.optJSONObject("android");if(a==null)return;primary.setEnabled(false);repair.setEnabled(false);status.setText("Preparing verified download…");io.execute(()->{try{File f=StoreCore.download(this,a.getString("apkURL"),a.getLong("bytes"),a.getString("sha256"),(d,t)->runOnUiThread(()->status.setText("Downloading securely… "+Math.min(100,(int)(100*d/Math.max(1,t)))+"%")));runOnUiThread(()->{status.setText("Verified. Android will ask you to approve installation.");primary.setEnabled(true);repair.setEnabled(true);StoreCore.install(this,f);});}catch(Exception e){runOnUiThread(()->{status.setText("Installation stopped: "+e.getMessage());primary.setEnabled(true);repair.setEnabled(true);});}});}
