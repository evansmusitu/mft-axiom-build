...
OUT = Path(os.environ.get("LIVE_MODULE_OUTPUT", "/tmp/musitu-store-exact-live"))
OUT.mkdir(parents=True, exist_ok=True)
AUTH = {
    "X-Auth-Email": os.environ["CLOUDFLARE_EMAIL"],
    "X-Auth-Key": os.environ["CLOUDFLARE_GLOBAL_API_KEY"],
    "Accept": "*/*",
    "User-Agent": "MUSITU-Store-Exact-Live-Capture/1.0",
}
EXPECTED = {
...
def main() -> None:
    ...
    for name, expected in EXPECTED.items():
        ...
        digest = sha256(parts[name])
        if digest != expected:
            raise RuntimeError(f"live {name} drift: {digest} != {expected}")
        (OUT / name).write_bytes(parts[name])
        observed[name] = {"sha256": digest, "bytes": len(parts[name])}

    report = {
        "schema": "musitu.store.phase2.exact_live_worker_capture.v1",
        "result": "PASS_EXACT_LIVE_CAPTURE",
